...
def _gamma_t_greedy(G):
    """Greedy approximation for total domination number (every node needs a neighbour in S)."""
    nodes = list(G.nodes())
    n = len(nodes)
    if n < 2:
        return n
    all_nodes = set(nodes)
    dominated = set()  # vertices that have a neighbour in S
    S = set()
    # pre-compute neighbourhoods
    nbr = {v: set(G.neighbors(v)) for v in nodes}
    while dominated != all_nodes:
        remaining = all_nodes - dominated
        if not remaining:
            break
        # Among ALL vertices (can re-add), pick the one covering most un-dominated
        best = max(all_nodes - S, key=lambda v: len(nbr[v] - dominated), default=None)
        if best is None:
            break
        gain = nbr[best] - dominated
        if not gain:
            # pick any undominated vertex's neighbour
            for u in remaining:
                candidates = nbr[u]
                if candidates:
...
def _iota_exact(G):
    """Exact independent domination number = minimum maximal independent set."""
    nodes = list(G.nodes())
    n = len(nodes)
    if n == 0:
        return 0
    ...
    for size in range(1, n + 1):
        for S_tuple in combinations(range(n), size):
            S = frozenset(S_tuple)
            indep = all(
                S_tuple[j] not in adj[S_tuple[i]]
                for i in range(len(S_tuple))
                for j in range(i + 1, len(S_tuple))
            )
            if not indep:
                continue
            dominated = frozenset(i for i in range(n) if i in S or (adj[i] & S))
            if dominated == all_n:
                return size
    return n
...
def _iota_greedy(G):
    """Greedy approximation for independent domination number."""
    nodes = list(G.nodes())
    nbr = {v: set(G.neighbors(v)) for v in nodes}
    available = set(nodes)
    indep_dom = set()
    while available:
        v = min(available, key=lambda u: len(nbr[u] & available))
        indep_dom.add(v)
        available -= {v} | nbr[v]
    return max(1, len(indep_dom))
...
def exact_parameters(G):
    ...
    if n <= 18:
        ...
        props["gamma_t"] = _gamma_t_greedy(G)

    # Calcul exact de iota (independent domination number) pour petits graphes.
    if n <= 16:
        props["iota"] = _iota_exact(G)
    else:
        props["iota"] = _iota_greedy(G)
...
def eval_poly(c, x_val):
    ...
    for i, coef in enumerate(coeffs):
        result += float(coef) * (float(x_val) ** (i + 1))
...
def _subgroup_tokens(subgroup):
    ...
    if isinstance(subgroup, str):
        ...
        raw_items = list(subgroup)

    tokens = []

    for item in raw_items:
        text = str(item).lower().strip().strip("[](){}'\"")
    ...
    return tokens or ["connected"]


def _graph_class(subgroup):
    tokens = _subgroup_tokens(subgroup)

    for sg in ("tree", "claw_free", "bipartite", "planar"):
        if sg in tokens:
            return sg
...
def repair_graph(G, subgroup, rng):
    ...
    if sg == "tree":
        ...
        max_repairs = max(1, G.number_of_nodes() ** 3)
    ...
    if is_in_class(G, subgroup):
        return G

    # Strict fallback: return a guaranteed member of the requested class
    # rather than letting an invalid mutant enter the search population.
    n = max(2, min(G.number_of_nodes(), 12))
    ...
    if sg == "claw_free":
        if nx.is_connected(G):
            H = nx.convert_node_labels_to_integers(nx.line_graph(G))
            if H.number_of_nodes() > 0 and is_in_class(H, subgroup):
                return H
    ...
    if comps:
        H = G.copy()
        for i in range(len(comps) - 1):
            H.add_edge(next(iter(comps[i])), next(iter(comps[i + 1])))
...
def choose_mutation_profile(c):
    ...
    if "avg" in involved or "delta" in involved or "Delta" in involved or "omega" in involved:
        return "density"
...
def clique_with_private_leaves(clique_size, leaf_count=None):
    G = nx.complete_graph(clique_size)
    leaf_count = clique_size if leaf_count is None else leaf_count

    for i in range(leaf_count):
        G.add_edge(i % clique_size, clique_size + i)

    return nx.convert_node_labels_to_integers(G)


def complete_multipartite_balanced(n, parts):
    buckets = [[] for _ in range(parts)]

    for v in range(n):
        buckets[v % parts].append(v)
...
def path_with_private_leaves(path_len):
    G = nx.path_graph(path_len)
    next_id = path_len

    for v in range(0, path_len, 2):
...
def generate_regular_candidates(max_n, rng, max_degree=8):
    graphs = []

    for n in range(6, max_n + 4):
        for d in range(2, min(max_degree, n - 1) + 1):
            if n * d % 2 != 0:
                continue
...
def generate_density_candidates(max_n, rng):
    ...
    for n in range(5, max_n + 4):
        ...
        for parts in range(2, min(6, n)):
            graphs.append(complete_multipartite_balanced(n, parts))
        ...
        for p in [0.12, 0.18, 0.25, 0.40, 0.60, 0.80, 0.90, 0.95]:
            H = nx.gnp_random_graph(n, p, seed=rng.randint(0, 999999))
            if nx.is_connected(H):
                graphs.append(H)

    graphs.extend(generate_regular_candidates(max_n, rng, max_degree=10))
...
def generate_distance_candidates(max_n):
    ...
    for n in range(5, max_n + 4):
        ...
        graphs.append(nx.star_graph(n - 1))
        graphs.append(nx.cycle_graph(n))
        graphs.append(path_with_private_leaves(n))
...
def generate_clawfree_candidates(max_n, rng):
    ...
    for n in range(3, max_n + 4):
        graphs.append(nx.path_graph(n))
        graphs.append(nx.complete_graph(n))
        graphs.append(nx.cycle_graph(max(3, n)))

    for clique_size in range(4, min(max_n + 1, 12)):
        for leaves in range(1, clique_size + 1):
            G = clique_with_private_leaves(clique_size, leaves)
            if G.number_of_nodes() <= max_n + 3:
                graphs.append(G)
    ...
    return [
        nx.convert_node_labels_to_integers(G)
        for G in graphs
        if G.number_of_nodes() > 0 and nx.is_connected(G) and _is_claw_free(G)
    ]
...
def generate_spectral_candidates(max_n, rng):
    ...
    for n in range(5, max_n + 4):
        ...
        for parts in range(2, min(7, n)):
            graphs.append(complete_multipartite_balanced(n, parts))

        for clique_size in range(4, min(n, 11)):
            G = clique_with_private_leaves(clique_size, n - clique_size)
        ...
        for p in [0.20, 0.35, 0.50, 0.70, 0.85, 0.92, 0.96]:
...
def corona_graph(H):
    """Corona H ⊙ K1 : attach one pendant to every vertex of H."""
    G = H.copy()
    n = H.number_of_nodes()
    nodes = list(H.nodes())
    for i, v in enumerate(nodes):
...
def subdivided_star(leaves, subdivisions=1):
    """Star K_{1,leaves} with each edge subdivided `subdivisions` times."""
    G = nx.Graph()
    center = 0
    nxt = 1
...
def double_corona(base_n):
    """Each vertex of path gets two pendants → gamma_t is large."""
    G = nx.path_graph(base_n)
    nxt = base_n
    for v in range(base_n):
        G.add_edge(v, nxt); G.add_edge(v, nxt + 1)
        nxt += 2
    return nx.convert_node_labels_to_integers(G)
...
def generate_domination_candidates(max_n, rng):
    ...
    for n in range(3, max_n + 4):
        ...
        if n >= 3:
            graphs.append(nx.cycle_graph(n))
        graphs.append(nx.star_graph(max(1, n - 1)))
        graphs.append(path_with_private_leaves(n))

        # Corona graphs — very high gamma_t
        for base_n in range(2, max(3, n // 2) + 1):
            for base_G in [nx.path_graph(base_n), nx.cycle_graph(max(3, base_n)),
                           nx.complete_graph(base_n)]:
                C = corona_graph(base_G)
                if 2 <= C.number_of_nodes() <= max_n + 3:
                    graphs.append(C)

        # Subdivided stars
        for leaves in range(2, min(8, n)):
            for sub in range(1, 4):
                G = subdivided_star(leaves, sub)
                if 2 <= G.number_of_nodes() <= max_n + 3:
                    graphs.append(G)

        # Double corona (paths with 2 pendants each)
        for base_n in range(2, min(8, n // 2 + 2)):
            G = double_corona(base_n)
            if 2 <= G.number_of_nodes() <= max_n + 3:
                graphs.append(G)
        ...
        # Random sparse graphs (good for domination)
        for p in [0.10, 0.15, 0.20, 0.30]:
            H = nx.gnp_random_graph(n, p, seed=rng.randint(0, 999999))
            if nx.is_connected(H):
                graphs.append(H)
...
def generate_priority_candidates(c, sg, max_n):
    involved = involved_invariants(c)
    local_rng = random.Random(1000 + int(c.get("id", 0)))
    candidates = []
    max_nodes = max_n + 3
    ...
    if "t" in involved:
        ...
        for a, b in [
            (half, half),
            (half, max_nodes - half - 1),
            (half - 1, half + 1),
            (half - 2, half + 2),
        ]:
            if a >= 3 and b >= 3:
                for path_len in range(0, 3):
                    G = two_cliques_bridge(a, b, path_len)
                    if G.number_of_nodes() <= max_nodes:
                        candidates.append(G)

        for a in range(min(15, max_nodes), 2, -1):
            for b in range(min(15, max_nodes), 2, -1):
                for path_len in range(0, 5):
                    G = two_cliques_bridge(a, b, path_len)
                    if G.number_of_nodes() <= max_nodes:
                        candidates.append(G)

        for clique_size in range(min(12, max_nodes), 2, -1):
            G = nx.Graph()
            previous_anchor = None
            next_node = 0

            while next_node + clique_size <= max_nodes:
                ...
                if previous_anchor is not None:
                    G.add_edge(previous_anchor, clique_nodes[0])

                previous_anchor = clique_nodes[-1]
                next_node += clique_size

            if G.number_of_nodes() > 0 and nx.is_connected(G):
                candidates.append(nx.convert_node_labels_to_integers(G))

    if {"omega", "delta", "Delta", "avg", "density"} & involved:
        for n in range(4, max_nodes + 1):
            candidates.append(nx.complete_graph(n))

            for parts in range(2, min(7, n)):
                candidates.append(complete_multipartite_balanced(n, parts))

            for clique_size in range(3, min(n, 12)):
                G = clique_with_private_leaves(clique_size, n - clique_size)
                if G.number_of_nodes() == n:
                    candidates.append(G)

    if {"remoteness", "proximity", "diam", "rad"} & involved:
        for n in range(2, max_nodes + 1):
            candidates.append(nx.path_graph(n))
            if n >= 3:
                candidates.append(nx.cycle_graph(n))
            candidates.append(path_with_private_leaves(n))

    if {"gamma", "gamma_t", "mu", "tau", "alpha", "iota"} & involved:
        for n in range(2, max_nodes + 1):
            candidates.append(nx.path_graph(n))
            if n >= 3:
                candidates.append(nx.cycle_graph(n))
            ...
            candidates.append(clique_with_private_leaves(clique_size, n - clique_size))
            # Corona graphs: gamma_t can be very high
            for base_n in range(2, min(n, 10)):
                for base_G in [nx.path_graph(base_n),
                               nx.cycle_graph(max(3, base_n)),
                               nx.complete_graph(base_n)]:
                    C = corona_graph(base_G)
                    if C.number_of_nodes() <= max_nodes:
                        candidates.append(C)
            # Subdivided stars
            for leaves in range(2, min(7, n)):
                for sub in range(1, 4):
                    G = subdivided_star(leaves, sub)
                    if G.number_of_nodes() <= max_nodes:
                        candidates.append(G)
            # Double corona
            for base_n in range(2, min(7, n // 2 + 2)):
                G = double_corona(base_n)
                if G.number_of_nodes() <= max_nodes:
                    candidates.append(G)

    if {"largest_eigenvalue", "t"} & involved:
        for n in range(4, max_nodes + 1):
            candidates.append(nx.complete_graph(n))
            for parts in range(2, min(7, n)):
                candidates.append(complete_multipartite_balanced(n, parts))
            for clique_size in range(4, min(n, 12)):
                G = clique_with_private_leaves(clique_size, n - clique_size)
                if G.number_of_nodes() == n:
                    candidates.append(G)

        candidates.extend(generate_regular_candidates(max_n, local_rng, max_degree=10))

    if sg == "tree":
        candidates = [G for G in candidates if nx.is_tree(G)]

    elif sg == "claw_free":
        extra = []
        for n in range(2, max_nodes + 1):
            extra.append(nx.path_graph(n))
            if n >= 3:
                extra.append(nx.cycle_graph(n))
            extra.append(nx.complete_graph(n))
            for clique_size in range(3, min(n, 12)):
                G = clique_with_private_leaves(clique_size, n - clique_size)
                if G.number_of_nodes() == n:
                    extra.append(G)
        candidates.extend(extra)
        candidates = [G for G in candidates if nx.is_connected(G) and _is_claw_free(G)]

    elif sg == "bipartite":
        candidates = [G for G in candidates if nx.is_connected(G) and nx.is_bipartite(G)]

    elif sg == "planar":
        candidates = [G for G in candidates if nx.is_connected(G) and nx.is_planar(G)]

    result = []
    seen = set()

    for G in candidates:
        if G.number_of_nodes() < 1 or G.number_of_nodes() > max_nodes:
            continue
        if not nx.is_connected(G):
            continue
        ...
        try:
            key = nx.to_graph6_bytes(H, header=False).decode().strip()
        except Exception:
            key = (H.number_of_nodes(), H.number_of_edges(), tuple(sorted(H.degree())))
...
def generate_initial_bank(max_n, rng):
    ...
    # Friendship graphs (windmill): k triangles sharing one center vertex
    for k in range(1, 9):
        G = nx.Graph()
        G.add_node(0)
        for i in range(k):
        ...
        if G.number_of_nodes() <= max_n + 3:
            bank.append(nx.convert_node_labels_to_integers(G))

    # Cocktail party graphs (complement of n*K2)
    for cp in range(2, 8):
        try:
            G = nx.cocktail_party_graph(cp)
            if nx.is_connected(G) and G.number_of_nodes() <= max_n + 3:
                bank.append(G)
        except Exception:
            pass

    # Circulant graphs
    for n in range(5, min(max_n + 1, 17)):
        for offsets in [[1, 2], [1, 3], [1, 2, 3], [2, 3], [1, 4], [1, 2, 4]]:
            try:
    ...
    # Grid graphs
    for r in range(2, 6):
        for c_g in range(2, 6):
            G = nx.convert_node_labels_to_integers(nx.grid_2d_graph(r, c_g))
            if G.number_of_nodes() <= max_n + 3:
                bank.append(G)

    # Turán graphs (max edges with no (r+1)-clique) - good for density vs triangle conjectures
    for n in range(4, min(max_n + 1, 16)):
    ...
    # Book graphs B_n (n triangles sharing a common edge)
    for k in range(1, 8):
        G = nx.Graph()
        G.add_edge(0, 1)
        for i in range(k):
    ...
    # Kneser and Johnson graphs (good for independence number)
    for n in range(3, 8):
        for k in range(1, n):
            try:
    ...
    # Almost-complete graphs (K_n minus a matching) - good for density near 1
    for n in range(4, min(max_n + 1, 14)):
        G = nx.complete_graph(n)
        edges = list(G.edges())
        for e in edges[:n//2]:
            ...
            if nx.is_connected(G2):
                bank.append(nx.convert_node_labels_to_integers(G2))
                break

    # Bipartite graphs with various densities (good for density/triangle conjectures)
    for a in range(1, 9):
        for b in range(a, 9):
            if a + b <= max_n + 3:
                bank.append(nx.complete_bipartite_graph(a, b))
...
def _mutate_claw_free(G, max_n, rng, profile="general"):
    original = G.copy()
    G = G.copy()
    n = G.number_of_nodes()
    nodes = list(G.nodes())
    ...
    ops = [
        ...
        "add_true_twin",
        "add_line_graph_step",
        "densify_neighborhood",
    ]

    if profile in ("density", "spectral", "degree_index"):
        ops += ["add_edge", "densify_neighborhood", "add_true_twin"]

    if profile in ("distance", "domination"):
        ops += ["remove_vertex", "add_line_graph_step"]
    ...
    if op == "add_edge":
    ...
    elif op == "add_true_twin" and n < max_n + 3:
        v = rng.choice(nodes)
        new_v = max(nodes) + 1
        G.add_edge(new_v, v)

        for u in list(original.neighbors(v)):
            G.add_edge(new_v, u)

    elif op == "add_line_graph_step" and edges:
        LG = nx.convert_node_labels_to_integers(nx.line_graph(G))

        if 1 <= LG.number_of_nodes() <= max_n + 3 and nx.is_connected(LG):
            G = LG

    elif op == "densify_neighborhood":
        centers = nodes[:]
        rng.shuffle(centers)
        ...
        for v in centers:
            ...
            if len(nbrs) >= 2:
                missing = [(a, b) for a, b in combinations(nbrs, 2) if not G.has_edge(a, b)]

                if missing:
                    G.add_edge(*rng.choice(missing))
                    break

    G = nx.convert_node_labels_to_integers(G)

    if nx.is_connected(G) and _is_claw_free(G):
        return G
...
def _single_mutate(G, sg, max_n, rng, profile="general"):
    ...
    if sg == "claw_free":
        return _mutate_claw_free(G, max_n, rng, profile)
...
def _exhaustive_atlas_search(c, subgroup, max_n):
    """Test ALL connected graphs from the graph atlas (up to 7 nodes) plus all small paths/cycles."""
    candidates = []
    try:
        for G in nx.graph_atlas_g():
            if 1 <= G.number_of_nodes() <= min(max_n + 3, 7) and nx.is_connected(G):
                candidates.append(nx.convert_node_labels_to_integers(G))
    except Exception:
        pass
    # Add K1 explicitly
    candidates.append(nx.complete_graph(1))
    # Also add slightly larger canonical structures
    for n in range(2, min(max_n + 4, 12)):
        candidates.append(nx.path_graph(n))
        if n >= 3:
            candidates.append(nx.cycle_graph(n))
        candidates.append(nx.complete_graph(n))
        candidates.append(nx.star_graph(max(1, n - 1)))
        # corona of path
        if n >= 2:
            candidates.append(corona_graph(nx.path_graph(n)))
        if n >= 3:
            candidates.append(corona_graph(nx.cycle_graph(n)))
        candidates.append(path_with_private_leaves(n))
        for sub in range(1, 4):
            for leaves in range(2, min(6, n)):
                G = subdivided_star(leaves, sub)
                if G.number_of_nodes() <= max_n + 3:
                    candidates.append(G)
        for base_n in range(2, min(7, n)):
            G = double_corona(base_n)
            if G.number_of_nodes() <= max_n + 3:
                candidates.append(G)
    # K1 must be included (n>=1, not n>=2)
    return [
        nx.convert_node_labels_to_integers(G)
        for G in candidates
        if G is not None and G.number_of_nodes() >= 1
        and nx.is_connected(G)
        and is_in_class(G, subgroup)
    ]
...
def solve_one(c, initial_bank, seconds, max_n, rng):
    ...
    # Trier class_bank : petits graphes en premier pour trouver K1/K2 rapidement
    class_bank_sorted = sorted(
        [G for G in initial_bank if is_in_class(G, subgroup)],
        key=lambda g: g.number_of_nodes()
    )
    class_bank = (specialized_start + class_bank_sorted)[:700]
    ...
    # ── PHASE -1 : test trivial graphes EN PREMIER (K1, K2, K3) ──────────────
    # Critique pour les conjectures dont le contre-exemple est un graphe trivial.
    _trivials = [
        nx.complete_graph(1),  # K1
        nx.complete_graph(2),  # K2
        nx.complete_graph(3),  # K3
        nx.path_graph(3),      # P3
        nx.cycle_graph(3),     # C3
        nx.path_graph(2),      # P2 = K2
    ...
    for _G in _trivials:
        try:
            ...
            if not is_in_class(_G, subgroup):
                continue
            _pp = exact_parameters(_G)
            _vv = violation(c, _pp)
            if _vv > VIOLATION_THRESHOLD:
                return _make_result(c, _G, _pp, _vv, time.time() - t0, "FOUND", "trivial_first")
            _fn, _s = choose_score(_G, _pp, c)
            population.append((_s, _G.copy(), _pp, _fn))
            if _s > best_score:
                best_score = _s; best_G = _G.copy(); best_props = _pp
                best_viol = _vv; best_name = _fn
        except Exception:
            pass

    for idx, G in enumerate(generate_priority_candidates(c, sg, max_n)):
        if idx > 0 and time.time() - t0 > seconds * 0.20:
            break

        result = add_candidate_to_population(G, c, subgroup, rng, population, max_n, t0)

        if result and result[0] == "FOUND":
            _, GG, pp, vv = result
            return _make_result(c, GG, pp, vv, time.time() - t0, "FOUND", "priority_candidate")

    # Phase 0.5 : exhaustive atlas scan (prioritaire pour les conjectures dures)
    for G in _exhaustive_atlas_search(c, subgroup, max_n):
        if time.time() - t0 > seconds * 0.18:
            break
        try:
            props = exact_parameters(G)
            viol = violation(c, props)
            if _valid_counterexample(G, props, c, subgroup):
                return _make_result(c, G, props, viol, time.time() - t0, "FOUND", "exhaustive_atlas")
            name, s = choose_score(G, props, c)
            population.append((s, G.copy(), props, name))
            if s > best_score:
                best_score = s; best_G = G.copy(); best_props = props
                best_viol = viol; best_name = name
        except Exception:
            pass

    # Injection de diversite forcee : si meilleur graphe est trivial (n<=2),
    # tester des structures variees pour echapper au minimum local.
    if best_G is None or best_G.number_of_nodes() <= 2:
        _FORCED_DIVERSITY = []
        for _n in range(3, min(max_n + 1, 16)):
            _FORCED_DIVERSITY.append(nx.complete_bipartite_graph(_n // 2, _n - _n // 2))
            _FORCED_DIVERSITY.append(nx.cycle_graph(_n))
            _FORCED_DIVERSITY.append(nx.path_graph(_n))
            if _n >= 5:
                # Friendship graph
                _fg = nx.Graph()
                _fg.add_node(0)
                for _i in range(_n // 2):
                    _fg.add_edge(0, 2*_i+1); _fg.add_edge(0, 2*_i+2); _fg.add_edge(2*_i+1, 2*_i+2)
                if _fg.number_of_nodes() <= max_n + 3:
                    _FORCED_DIVERSITY.append(nx.convert_node_labels_to_integers(_fg))
            # Turán K_{n, floor(n/2)}
            try:
                _FORCED_DIVERSITY.append(nx.turan_graph(_n, max(2, _n // 2)))
            except Exception:
                pass
            # Circulant
            try:
                _FORCED_DIVERSITY.append(nx.circulant_graph(_n, [1, 2]))
            except Exception:
                pass
            # Corona of path
            _cp = corona_graph(nx.path_graph(max(2, _n // 2)))
            if _cp.number_of_nodes() <= max_n + 3:
                _FORCED_DIVERSITY.append(_cp)
        for _G in _FORCED_DIVERSITY:
            if time.time() - t0 > seconds * 0.30:
                break
            try:
                _G = repair_graph(_G, subgroup, rng)
                if _G.number_of_nodes() < 2 or not is_in_class(_G, subgroup):
                    continue
                _props = exact_parameters(_G)
                _viol = violation(c, _props)
                if _valid_counterexample(_G, _props, c, subgroup):
                    return _make_result(c, _G, _props, _viol, time.time() - t0, "FOUND", "forced_diversity")
                _name, _s = choose_score(_G, _props, c)
                population.append((_s, _G.copy(), _props, _name))
                if _s > best_score:
                    best_score = _s; best_G = _G.copy(); best_props = _props
                    best_viol = _viol; best_name = _name
            except Exception:
                pass

    # Phase 1 : banque initiale + candidats spécialisés légers.
    for G in class_bank:
        if time.time() - t0 > seconds * 0.35:
            break
    ...
    while time.time() - t0 < seconds:
        ...
        stagnation_limit = 6.0 if restart_count < 3 else 4.0
        if now - last_improvement > stagnation_limit and now - last_restart > stagnation_limit:
            restart_count += 1
            population = population[:15]

            # Alternate restart strategies
            if restart_count % 3 == 0:
                # Full atlas re-scan
                extra = _exhaustive_atlas_search(c, subgroup, max_n)
                label = "restart_atlas"
            elif restart_count % 3 == 1:
                extra = generate_specialized_candidates(c, sg, max_n, rng)
                label = "restart_specialized"
            else:
                # Pure random with different seeds
                extra = [_fresh_candidate(sg, max_n, rng) for _ in range(80)]
                label = "restart_random"
            ...
            for G0 in extra[:120]:
                ...
                if result and result[0] == "FOUND":
                    _, GG, pp, vv = result
                    return _make_result(
                        c, GG, pp, vv, time.time() - t0, "FOUND", label,
                    )
        ...
        try:
            ...
            if len(population) < 160 or s > population[-1][0]:
