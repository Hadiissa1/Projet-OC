...
def _gamma_exact(G):
    ...
    return _cover_min_size(covers, n, min_size=1)
...
def _gamma_t_exact(G):
    ...
    if any(mask == 0 for mask in covers):
        return n

    return _cover_min_size(covers, n, min_size=2)


def _cover_min_size(covers, n, min_size=1):
    all_mask = (1 << n) - 1
    ...
    def greedy_upper():
        ...
        while covered != all_mask:
            best = max(covers, key=lambda mask: (mask & ~covered).bit_count())
        ...
        return max(min_size, used)

    best = [greedy_upper()]
    coverers = [[] for _ in range(n)]

    for i, mask in enumerate(covers):
        for v in range(n):
            if mask & (1 << v):
                coverers[v].append(i)

    def lower_bound(covered):
        uncovered_count = (all_mask & ~covered).bit_count()
        max_gain = max((mask & ~covered).bit_count() for mask in covers)
        return math.ceil(uncovered_count / max_gain) if max_gain else n
    ...
    def bt(covered, chosen):
        ...
        if covered == all_mask:
            best[0] = max(min_size, chosen)
            return

        if chosen + lower_bound(covered) >= best[0]:
            return

        uncovered = all_mask & ~covered
        target = min(
            (v for v in range(n) if uncovered & (1 << v)),
            key=lambda v: len(coverers[v])
        )

        options = sorted(
            coverers[target],
            key=lambda i: (covers[i] & ~covered).bit_count(),
            reverse=True
        )

        for i in options:
            bt(covered | covers[i], chosen + 1)
...
def solve_one(c, initial_bank, seconds, max_n, rng):
    ...
    class_bank = (specialized_start + class_bank)[:700]
